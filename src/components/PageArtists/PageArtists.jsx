...
export default function PageArtists({ selectedArtists }) {
  ...
  const { infoAlbums, title, artist, cover, releaseDate, album, tracks, albums, singles, playlist, fansAlsoListen } = selectedArtists;
      ...
      <div style={{ color: 'white' }}>
        <div style={{ borderRadius: '15px', background: 'linear-gradient(to right,#0E9EEF,rgb(4, 58, 87))' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <button onClick={() => navigate('/Popular')} style={{ cursor: 'pointer', width: '100px', height: '100px', fontSize: 40, backgroundColor: 'transparent', border: 'none' }}>
              <ArrowLeftOutlined />
            </button>
          ...
          <div style={{ display: 'flex', gap: '30px', marginLeft: '50px', paddingBottom: '30px' }}>
            <img src={cover} alt={title} style={{ width: '250px', borderRadius: '10px', marginTop: '20px' }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10%' }}>
              ...
              <p style={{ color: '#fff', marginBottom: '10px', fontWeight: 700, fontSize: 15 }}>
                {tracks?.length || 0} songs&nbsp;&nbsp; <span style={{ color: 'red' }}>●</span> &nbsp;&nbsp;{getTotalDuration(tracks || [])}
              </p>
            </div>
            <div style={{ width: '300px', height: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', color: '#EE10B0', gap: '10px', cursor: 'pointer' }}>
                <h1>Play All</h1>
                <button style={{ padding: 1, fontSize: 60, backgroundColor: 'transparent', border: 'none', color: '#EE10B0', cursor: 'pointer' }}>
                  <PlayCircleOutlined />
                </button>
              </div>
            </div>
                ...
                {tracks.map((track, index) => (
                  <div key={index} style={{ width: '100%', backgroundColor: '#1F1F1F', borderRadius: '10px', margin: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', width: '100%', justifyContent: 'space-around', paddingLeft: '50px', gap: '145px' }}>
        ...
        {albums && (
          <div style={{ padding: '40px' }}>
            <h2 style={{ color: 'white', marginBottom: '20px' }}>Artist’s Albums</h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
              {albums.map((album, index) => (
                <div key={index} style={{ color: 'white', textAlign: 'center' }}>
                  <img src={album.cover} alt={album.title} style={{ width: '150px', height: '150px', borderRadius: '10px' }} />
                  <p style={{ marginTop: '10px', fontWeight: 600 }}>{album.title}</p>
                  <p style={{ fontSize: '14px' }}>{album.year}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {singles && (
          <div style={{ padding: '40px' }}>
            <h2 style={{ color: 'white', marginBottom: '20px' }}>Single Songs</h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
              {singles.map((single, index) => (
                <div key={index} style={{ color: 'white', textAlign: 'center' }}>
                  <img src={single.cover} alt={single.title} style={{ width: '150px', height: '150px', borderRadius: '10px' }} />
                  <p style={{ marginTop: '10px', fontWeight: 600 }}>{single.title}</p>
                  <p style={{ fontSize: '14px' }}>{single.releaseDate} • {single.duration}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {playlist && (
          <div style={{ padding: '40px' }}>
            <h2 style={{ color: 'white', marginBottom: '20px' }}>Artist’s Playlist</h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
              {playlist.map((item, index) => (
                <div key={index} style={{ color: 'white', textAlign: 'center' }}>
                  <img src={item.cover} alt={item.title} style={{ width: '150px', height: '150px', borderRadius: '10px' }} />
        ...
        {fansAlsoListen && (
          <div style={{ padding: '40px' }}>
            <h2 style={{ color: 'white', marginBottom: '20px' }}>Eminem Fans Also Listen To</h2>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
              {fansAlsoListen.map((fan, index) => (
                <div key={index} style={{ color: 'white', textAlign: 'center' }}>
                  <img src={fan.cover} alt={fan.artist} style={{ width: '120px', height: '120px', borderRadius: '60px' }} />
                  <p style={{ marginTop: '10px', fontWeight: 600 }}>{fan.artist}</p>
                </div>
              ))}
            </div>
          </div>
